//------------------------------------------------------------------------------
// <auto-generated>
//    Codice generato da un modello.
//
//    Le modifiche manuali a questo file potrebbero causare un comportamento imprevisto dell'applicazione.
//    Se il codice viene rigenerato, le modifiche manuali al file verranno sovrascritte.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DAL
{
    using System;
    using System.Collections.Generic;
    
    public partial class hlt_daticlinici_backup
    {
        public long seriale { get; set; }
        public string objectid { get; set; }
        public Nullable<System.DateTime> data_creazione { get; set; }
        public Nullable<System.DateTime> data_modifica { get; set; }
        public string nomeutente_creazione { get; set; }
        public string nomeutente_modifica { get; set; }
        public string versione { get; set; }
        public string locker { get; set; }
        public string idepisodio { get; set; }
        public string storiaclinica { get; set; }
        public string distrettianatomici { get; set; }
        public string gruppipatologie { get; set; }
        public string interventichirurgici { get; set; }
        public string notetxt { get; set; }
        public string pdfcreato { get; set; }
    }
}
